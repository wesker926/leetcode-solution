package com.wesker926.leetcode.algorithms.p1263.s2;
            ...
            while (!queue.isEmpty()) {
                ...
                for (int[] dir : DIRS) {
                    ...
                    if (nxt[0] < 0 || nxt[0] >= m || nxt[1] < 0 || nxt[1] >= n ||
                            ...
                            grid[nxt[0]][nxt[1]] == '#' || grid[nxt[2]][nxt[3]] == '#' || visited[encode(nxt)])
                        continue;
                    if (nxt[4] == cur[4]) queue.offer(nxt);
                    else temp.offer(nxt);
                }
            }
            queue = temp;
